import { prisma } from "@/lib/prisma";
import { redirect } from "next/navigation";
import { createCompany } from "../actions";

export default async function NewCompanyPage() {
  const companyTypes = await prisma.companyType.findMany({ orderBy: { name: "asc" } });

  async function handleSubmit(formData: FormData) {
    "use server";
    const result = await createCompany(formData);
    if (!result?.error && result?.id) redirect(`/companies/${result.id}`);
  }

  return (
    <div className="max-w-lg space-y-6">
      <h1 className="text-2xl font-bold">Nytt firma</h1>
      <form action={handleSubmit} className="space-y-4 rounded border bg-white p-6">
        <div>
          <label className="block text-sm font-medium text-gray-700">Navn</label>
          <input
            name="name"
            required
            className="mt-1 w-full rounded border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Type</label>
          <select
            name="companyTypeId"
            required
            className="mt-1 w-full rounded border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Velg type…</option>
            {companyTypes.map((t) => (
              <option key={t.id} value={t.id}>
                {t.name}
              </option>
            ))}
          </select>
        </div>
        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id="isInternalCompany"
            name="isInternalCompany"
            value="true"
            className="h-4 w-4 rounded border-gray-300"
          />
          <label htmlFor="isInternalCompany" className="text-sm text-gray-700">
            Intern (vår egen organisasjon)
          </label>
        </div>
        <div className="flex gap-3 pt-2">
          <button
            type="submit"
            className="rounded bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700"
          >
            Opprett firma
          </button>
          <a href="/companies" className="rounded border px-4 py-2 text-sm hover:bg-gray-50">
            Avbryt
          </a>
        </div>
      </form>
    </div>
  );
}
